...
class HealthSimulation:
    ...
    def print_monthly_cost_summaries(self, plan_names: Optional[List[str]] = None) -> None:
        """
        Print minimum, maximum, and average monthly costs for each plan.
        
        Args:
            plan_names: Optional list of plan names to analyze. If None, analyzes all plans.
        """
        plans_to_analyze = plan_names if plan_names else list(self.plans.keys())
        
        print("\nMonthly Cost Summary Statistics:")
        print("-" * 60)
        
        for plan_name in plans_to_analyze:
            if plan_name not in self.plans:
                print(f"Warning: Plan '{plan_name}' not found. Skipping.")
                continue
                
            plan_results = self.plans[plan_name].cost_results
            
            # Get monthly costs for all simulations
            monthly_costs = self._calculate_monthly_costs(plan_results)
            
            # Find minimum scenario (simulation with lowest maximum monthly cost)
            max_monthly_per_sim = monthly_costs.max(axis=1)  # Max monthly cost for each simulation
            min_scenario_idx = max_monthly_per_sim.argmin()  # Simulation with lowest max monthly cost
            min_value = max_monthly_per_sim[min_scenario_idx]  # The actual lowest max monthly cost
            
            # Find maximum scenario (simulation with highest single monthly cost)
            max_value = monthly_costs.max()  # Highest single monthly cost across all simulations
            
            # Calculate average monthly cost across all simulations and months
            avg_value = monthly_costs.mean()
            
            print(f"\n{plan_name}:")
            print(f"  Minimum (lowest max monthly): ${min_value:,.2f}")
            print(f"  Maximum (highest monthly):    ${max_value:,.2f}")
            print(f"  Average (overall):           ${avg_value:,.2f}")
    
    def plot_monthly_cost_analysis(self, plan_names: Optional[List[str]] = None) -> None:
        """
        Plot monthly costs and cumulative costs for minimum, maximum, and average scenarios.
        ...
        # Set up consistent colors for plans
        colors = plt.cm.tab10(np.linspace(0, 1, len(plans_to_analyze)))
        plan_colors = dict(zip(plans_to_analyze, colors))
        ...
        scenarios = ['Minimum', 'Maximum', 'Average']
        
        for scenario_idx, (ax, scenario) in enumerate(zip(axes, scenarios)):
            # Create twin axis for cumulative costs
            ax2 = ax.twinx()
            ...
            for plan_name in plans_to_analyze:
                ...
                if scenario == 'Minimum':
                    # Find simulation with lowest maximum monthly cost
                    max_monthly_per_sim = monthly_costs.max(axis=1)
                    scenario_sim_idx = max_monthly_per_sim.argmin()
                    scenario_monthly_costs = monthly_costs[scenario_sim_idx, :]
                    
                elif scenario == 'Maximum':
                    # Find simulation with highest single monthly cost
                    flat_costs = monthly_costs.flatten()
                    max_cost_idx = flat_costs.argmax()
                    scenario_sim_idx = max_cost_idx // 12  # Convert back to simulation index
                    scenario_monthly_costs = monthly_costs[scenario_sim_idx, :]
                    
                else:  # Average
                    # Average monthly costs across all simulations
                    scenario_monthly_costs = monthly_costs.mean(axis=0)
                
                # Calculate cumulative costs
                cumulative_costs = np.cumsum(scenario_monthly_costs)
                
                # Plot monthly costs as bars (with slight offset for multiple plans)
                x_pos = np.arange(12) + (list(plans_to_analyze).index(plan_name) * 0.8 / len(plans_to_analyze))
                ax.bar(x_pos, scenario_monthly_costs, width=0.8/len(plans_to_analyze), 
                       color=color, alpha=0.7, label=f'{plan_name}')
                
                # Plot cumulative costs as lines
                ax2.plot(range(12), cumulative_costs, color=color, linewidth=2, 
                         marker='o', markersize=4)
            
            # Customize subplot
            ax.set_title(f'{scenario} Scenario: Monthly and Cumulative Costs', fontsize=14, fontweight='bold')
            ax.set_xlabel('Month')
            ax.set_ylabel('Monthly Cost ($)', color='blue')
    ...
    def _calculate_monthly_costs(self, plan_results) -> np.ndarray:
        """
        Helper method to calculate monthly costs from daily cost data.
        
        Args:
            plan_results: xarray Dataset containing daily cost data
            
        Returns:
            numpy array of shape (n_simulations, 12) containing monthly costs
        """
        # Get dimensions
        n_simulations = plan_results.sizes['simulation']
        
        # Initialize monthly costs array
        monthly_costs = np.zeros((n_simulations, 12))
        
        # Get dates for month mapping
        dates = plan_results.day.values
        
        for sim in range(n_simulations):
            for month in range(1, 13):  # Months 1-12
                # Find days in this month
                month_mask = np.array([pd.Timestamp(date).month == month for date in dates])
                month_days = np.where(month_mask)[0]
                
                # Sum medical costs for this month across all family members
                month_medical = plan_results.medical_daily_costs.isel(
                    simulation=sim, day=month_days
                ).sum().item()
                
                # Add monthly premium (appears on first day of each month)
                month_premium = plan_results.premium_daily_costs.isel(
                    simulation=sim, day=month_days
                ).sum().item()
